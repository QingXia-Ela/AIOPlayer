...
import { createRef, FunctionComponent, PropsWithChildren, useEffect, useState } from 'react';
...
const SilverBorderButton: FunctionComponent<React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement>> = ({ children, ...p }) => {
  const svgWrapper = createRef<HTMLButtonElement>()
  const Border = createRef<SilverBorderMethods>()

  useEffect(() => {
    const w = svgWrapper.current!.clientWidth, h = svgWrapper.current!.clientHeight
  ...
  return (
    <button {...p} className={`${Styles.silver_button} ${p.className ?? ""}`} ref={svgWrapper}>
